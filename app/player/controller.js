...
module.exports = {
  ...
  detailPage: async (req, res) => {
    try {
      ...
      if (!Mongoose.Types.ObjectId.isValid(id)) {
        return res
          .status(400)
          .send({ message: "Voucher game tidak ditemukan.!" });
      }
  ...
  checkout: async (req, res) => {
    try {
      ...
      const price = res_nominal._doc.price;
  ...
  historyDetail: async (req, res) => {
    try {
      ...
      // Handling invalid ID format!
      if (!Mongoose.Types.ObjectId.isValid(id)) {
        return res.status(400).send({ message: "History tidak ditemukan.!" });
      }
    ...
    try {
      ...
      if (req.file) {
        ...
        src.on("end", async () => {
          ...
          const curentImage = `${config.rootPath}/public/uploads/${player.avatar}`;
